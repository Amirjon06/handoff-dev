...
Current capture output includes the Git repository root, origin remote, active branch, and current commit. Use `--json` to produce the first StateRelay session artifact. Restore currently verifies that the captured Git root still exists and the branch matches before printing a plan.
